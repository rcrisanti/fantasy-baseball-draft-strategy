...
def main():
    ...
    pitching_stats.dropna(
        how="all",
        subset=["w", "sv", "hld", "so", "er", "bb", "h", "ip", "g", "gs"],
        inplace=True,
    )
    ...
    pitching_stats_clean = pd.merge(
        ...
        pitching_stats.loc[
            :, ["player_id", "w", "sv", "hld", "so", "er", "bb", "h", "ip", "g", "gs"]
        ]
        .groupby("player_id")
        .sum(),
        how="right",
    ...
    pitching_stats_clean["SP"] = pitching_stats_clean["gs"] >= 3
    pitching_stats_clean["RP"] = (
        pitching_stats_clean["g"] - pitching_stats_clean["gs"]
    ) >= 5
    pitching_stats_clean.drop(columns=["primary_position", "g", "gs"], inplace=True)
    assert (
